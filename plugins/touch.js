  ...
  install(Vue) {
    Vue.directive('touch', {
      bind: (el, binding) => {
        if (!el.hammer) {
          el.hammer = new Hammer.Manager(el, {
            recognizers: [
              ...
              [
                Hammer.Swipe,
                { threshold: 100, direction: Hammer.DIRECTION_HORIZONTAL }]
            ]
          })
        }
        const mc = el.hammer
        ...
        if (!event)
          console.warn('[v-touch] event type argument is required.')
        ...
        if (this.customEvents[event]) {
          ...
          if (!recognizerType && !subGesturesType) {
            console.warn('[v-touch] invalid event type: ' + event)
            return
          }
          if (subGesturesType && el.__hammerConfig[subGesturesType].direction.length !== 0)
            console.warn('[v-touch] ' + subGesturesType + ' should not have directions')
          ...
          if (recognizerType === 'tap' || recognizerType === 'pinch' || recognizerType === 'press' || recognizerType === 'rotate') {
            if (el.__hammerConfig[recognizerType].direction.length !== 0)
              throw Error('[v-touch] ' + recognizerType + ' should not have directions')
          }
      ...
      inserted: (el, binding) => {
        ...
        if (typeof binding.value !== 'function') {
          mc.handler = null
          console.warn(
            '[v-touch] invalid handler function for v-touch: ' +
            binding.arg
          )
        } else {
  ...
  guardDirections(options) {
    ...
    if (typeof dir === 'string') {
      ...
      if (directions.indexOf(dir) > -1 && Hammer.hasOwnProperty(hammerDirection))
        ...
        console.warn('[v-touch] invalid direction: ' + dir)
    }
  },
